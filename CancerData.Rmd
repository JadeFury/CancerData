...
#cor(filcancer, pca2)
```
from plot we see that at the 7th component its almost 0% so we should pick up the first 7. also the cumulative sum over all the variance, you can see that the first 7 eigenvalues correspond to approximately 98% of the all variance. it means that there is just 2% of information being lost. so we are moving from 10 means to 7means
...
ggplot(df_radius, aes(x=rad_worst, y = rad_se))+geom_point()
ggplot(df_perimeter, aes(x=peri_worst, y = peri_se))+geom_point()
ggplot(df_area, aes(x=area_worst, y = area_se))+geom_point()
