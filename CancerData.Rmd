...
```{r}
# cross-validation
cvcancer.lda <- lda(diagnosis ~ radius_mean+texture_mean+perimeter_mean+area_mean+smoothness_mean+compactness_mean+concavity_mean+concave_points_mean+symmetry_mean+fractal_dimension_mean, data=can, CV=TRUE)
table(can$diagnosis, cvcancer.lda$class)
```
...
cancer.lda2 <- lda(diagnosis ~ ., data=can2, CV=FALSE)
cancer.lda2
```
```{r}
...
```{r}
# cross-validation
cvcancer.lda2 <- lda(diagnosis ~ ., data=can2, CV=TRUE)
table(can2$diagnosis, cvcancer.lda2$class)
```
