...
cancer.lda2 <- lda(diagnosis ~ ., data=can2, CV=FALSE, prior=rep(1/2,2))
cancer.lda2
...
#Attempted calculation of new Sp for discriminant scores
Sp <- matrix(0, p, p)
nx <- rep(0, g)
lev <- levels(can2$diagnosis)
for(k in 1:g){
x <- can2[can2$diagnosis==lev[k],2:p]
nx[k] <- nrow(x)
Sp <- Sp + cov(x) * (nx[k])
}
Sp <- Sp / (sum(nx) - g)
...
plot(dscores, xlab="LD1", pch=canid, col=canid,
